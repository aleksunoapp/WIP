export const getters = {
  ...
  deferredServices: (state, getters) => {
    let deferred = []
    ...
    deferred.sort((a, b) => {
      const aSortOrder = getters.categoryById(a.category).sortOrder
      const bSortOrder = getters.categoryById(b.category).sortOrder
  ...
  firstApprovedServices: (state, getters) => {
    let firstApprovedServices = []
        ...
        if (service.subServices) {
          service.subServices.forEach((subService) => {
            if (subService.isSelected) {
              firstApprovedServices.push(subService)
            }
          })
        } else {
          if (service.isSelected) {
            firstApprovedServices.push(service)
          }
        }
      }
    })

    return firstApprovedServices
  },
  total: (state, getters) => {

<template>
	<modal :show="showMenuTreeModal" :width="900" effect="fade" @closeOnEscape="closeModal">
		<div slot="modal-header" class="modal-header center">
			...
			<h4 v-if="updateType !== 'sku'" class="modal-title center">Apply <i>{{ headerText }}</i> To Multiple Items</h4>
			<h4 v-else class="modal-title center">{{ headerText }}</h4>
		</div>
...
export default {
	...
	methods: {
					...
					if (menuTreeVue.updateType === 'modifier') {
						for (var i = 0; i < menuTreeVue.items.length; i++) {
							if (menuTreeVue.items[i].modifiers.length) {
								for (var j = 0; j < menuTreeVue.items[i].modifiers.length; j++) {
									var modItem = menuTreeVue.items[i]
									if (modItem.modifiers[j].id === menuTreeVue.selectedObject.id) {
		...
		applyModifierToSelectedItems () {
			...
			ModifiersFunctions.applyModifierToMultipleItems(menuTreeVue.selectedObject.id, selectedItems, unselectedItems, menuTreeVue.$root.appId, menuTreeVue.$root.appSecret, menuTreeVue.$root.userToken).then(response => {
				if (response.code === 200 && response.status === 'ok') {
		...
		applyTagToSelectedItems () {
			...
			TagsFunctions.applyTagToMultipleItems(menuTreeVue.selectedObject.id, selectedItems, unselectedItems, menuTreeVue.$root.appId, menuTreeVue.$root.appSecret, menuTreeVue.$root.userToken).then(response => {
				if (response.code === 200 && response.status === 'ok') {
					menuTreeVue.closeModal()
				}
			}).catch(reason => {

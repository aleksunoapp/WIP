<template>
	<div>
		...
		<modal v-show="isModal" :show.sync="showResourceModal" effect="custom" :width="'96%'">
			...
			<div slot="modal-body" class="modal-body" :style="{'height': 'calc(100vh - 180px)', 'max-height': 'none'}">
				<div class="margin-top-20">
					<div class="margin-bottom-25 col-sm-4">
					...
					<div class="col-sm-8">
						...
						<div v-if="activeFolder.id || allFoldersView">
							<div class="resource-header clearfix">
								<file-upload v-if="!allFoldersView" :folderId="activeFolder.id" @savingUpdate="checkSaving" @success="imageUploaded"></file-upload>
								<button v-if="!isSaving" type="button" class="btn btn-sm btn-outline btn-default margin-bottom-10" @click="toggleFilterPanel()">{{ filterCollapse ? 'Show' : 'Hide' }} Filter and Search Options</button>
							</div>
							<div class="portlet box blue-hoki portlet-blue-override">
								<div class="portlet-body" :class="{'display-hide': filterCollapse}">
									<form role="form" @submit.prevent="advancedResourceSearch()">
										...
										<div class="filter-tags" v-if="tags.length">
											<label>Resource Tags <small>(Toggle the tags you want to filter by.)</small></label>
											<div>
												<template v-for="tag in tags">
													<button type="button" class="btn btn-default btn-xs" :class="{'blue': tag.active}" @click="toggleTag(tag)">{{ tag.name }}</button>
												</template>
											</div>
										</div>
										<div class="filter-tags" :class="{'margin-top-20': tags.length}">
											<label>File Types <small>(Toggle the types you want to filter by.)</small></label>
											<div>
												<template v-for="type in fileTypes">
													<button type="button" class="btn btn-default btn-xs" :class="{'blue': type.active}" @click="toggleFileType(type)"><i :class="type.icon"></i> {{ type.name }}</button>
												</template>
											</div>
										</div>
									...
									<div class="cbp-wrapper">
										...
										<div v-if="!loadingResourceData && !isSaving">
											<div class="clearfix">
												<dropdown>
													<button slot="button" type="button" class="btn btn-sm btn-outline dropdown-toggle btn-default" data-toggle="dropdown">
														Sort By<span v-if="sortBy.field"><b>: {{ sortOptions[sortBy.field] }} <i class="fa fa-sort-alpha-asc" v-if="sortBy.order === 'asc'"></i><i class="fa fa-sort-alpha-desc" v-if="sortBy.order === 'desc'"></i></b></span>
											...
											<template v-for="resource in currentResources">
												<div class="col-sm-4" ref="resource">
													<div @click="setResource(resource)" class="resource-wrapper clickable" :class="{'selected': selectedResource.id === resource.id}">
			...
			<div slot="modal-footer" class="modal-footer">
				<button type="button" class="btn btn-default" @click="closeResourceModal()">Close</button>
				<button type="button" class="btn btn-primary" @click="resourceSelectionComplete()" :disabled="!selectedResource.id">Done</button>
			</div>
		</modal>
		<section v-show="!isModal && showResourceModal">
			<div slot="modal-header" class="modal-header center">
				<h4 class="modal-title">Select a Resource</h4>
			</div>
			<div slot="modal-body" class="modal-body" :style="{'height': 'calc(100vh - 180px)', 'max-height': 'none'}">
...
export default {
	...
	methods: {
		...
		getFolders (parentId, folder) {
			...
			return Resources.getFolders(businessId, locationId, parentId, pageNumber, recordsPerPage).then(response => {
				if (parentId === 0) {
		...
		getResources (folder, pageNumber) {
			...
			return Resources.getResources(businessId, locationId, folderId, pageNumber, recordsPerPage, searchCriteria, sortCriteria).then(response => {
				...
				_this.preloadImages().then(response => {
					setTimeout(() => {

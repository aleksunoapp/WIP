	...
	<div>
		<div>
			...
			<div class="portlet box blue-hoki margin-top-20">
				...
				<div class="portlet-body" :class="{'display-hide': createLocationManagerCollapse}">
	      				...
	      				<div class="row">
	      					...
	      					<div class="col-md-6">
	      						<div class="form-group form-md-line-input form-md-floating-label">
	      						    <input ref="newLocationManagerName" type="text" class="form-control input-sm" id="form_control_name" v-model="newLocationManager.name" :class="{'edited': newLocationManager.name.length}">
	      						    <label for="form_control_name">Name</label>
	      						</div>
	      						<div class="form-group form-md-line-input form-md-floating-label">
	      						    <input type="text" class="form-control input-sm" id="form_control_phone" v-model="newLocationManager.phone" :class="{'edited': newLocationManager.phone.length}">
	      						    <label for="form_control_phone">Phone</label>
	      						</div>
	      						<div class="form-group form-md-line-input form-md-floating-label">
	      						    <input type="text" class="form-control input-sm" id="form_control_email" v-model="newLocationManager.email" :class="{'edited': newLocationManager.email.length}">
	      					...
	      					<div class="col-md-6">
	      						<div class="form-group form-md-line-input form-md-floating-label">
	      							<div class="input-group" v-show="passwordMasked">
		      						    <input type="password" class="form-control input-sm" id="form_control_password_masked" v-model="newLocationManager.password" :class="{'edited': newLocationManager.password.length}">
	      						...
	      						<div class="form-group form-md-line-input form-md-floating-label">
	      							<div class="input-group" v-show="passwordMasked">
		      						    <input type="password" class="form-control input-sm" id="form_control_confirm" v-model="passwordCheck" :class="{'edited': passwordCheck}">
	      							...
	      							<div class="input-group" v-show="!passwordMasked">
		      						    <input type="text" class="form-control input-sm" id="form_control_confirm_masked" v-model="passwordCheck" :class="{'edited': passwordCheck}">
	        ...
	        <!-- SEARCH START -->
            <div class="margin-top-20" v-if="locationManagers.length">
			...
			<!-- LIST START -->
	        <loading-screen :show="loadingLocationManagersData" :color="'#2C3E50'" :display="'inline'"></loading-screen>
			        ...
			        <div class="portlet-body">
        				<div class="clearfix margin-bottom-10" v-if="locationManagers.length">
        					<el-dropdown trigger="click" @command="updateSortByOrder" size="mini" :show-timeout="50" :hide-timeout="50">
        	  				...
        	  				<page-results class="pull-right" :totalResults="locationManagers.length" :activePage="activePage" @pageResults="pageResultsUpdate"></page-results>
        				</div>
			            <div class="mt-element-list">
			                ...
			                <div class="clearfix" v-if="locationManagers.length && numPages > 1">
	        ...
	        <div v-if="!locationManagers.length && !loadingLocationManagersData">
	        	<no-results :show="!locationManagers.length" :type="'location managers'"></no-results>
	        </div>
		</div>
		<!-- LIST END -->
...
export default {
	...
	computed: {
		...
		currentActivePageItems () {
			return this.userSort(this.locationManagers).slice(this.resultsPerPage * (this.activePage - 1), this.resultsPerPage * (this.activePage - 1) + this.resultsPerPage)
		},
		searchNumPages () {
	...
	methods: {
			...
			if (this.searchTerm.length) {
				if (this.searchTerm.length < 3) {
					...
					for (var i = 0; i < this.locationManagers.length; i++) {
						if ((this.locationManagers[i].name.toLowerCase().indexOf(this.searchTerm.toLowerCase()) > -1) || (this.locationManagers[i].email.toLowerCase().indexOf(this.searchTerm.toLowerCase()) > -1)) {
		...
		assignStores (storesToBeAssigned) {
			...
			AdminManagerFunctions.assignStores(payload, assignStoresVue.$root.appId, assignStoresVue.$root.appSecret, assignStoresVue.$root.userToken).then(response => {
				if (response.code === 200 && response.status === 'ok') {
					...
					assignStoresVue.animated = `locationManager-${assignStoresVue.selectedLocationManager.id}`
					window.setTimeout(() => {
		...
		getAllLocationManagers () {
			...
			var locationManagersVue = this
			return AdminManagerFunctions.getAllAdmins(locationManagersVue.$root.appId, locationManagersVue.$root.appSecret, locationManagersVue.$root.userToken).then(response => {
				if (response.code === 200 && response.status === 'ok') {
			...
			.then((response) => {
				...
				return AdminManagerFunctions.updateAdmin(locationManagersVue.locationManagerToBeEdited, locationManagersVue.$root.appId, locationManagersVue.$root.appSecret, locationManagersVue.$root.userToken).then(response => {
					if (response.code === 200 && response.status === 'ok') {
						...
						for (var i = 0; i < this.locationManagers.length; i++) {
							if (this.locationManagers[i].id === locationManagersVue.locationManagerToBeEdited.id) {
								this.locationManagers[i].name = locationManagersVue.locationManagerToBeEdited.name
								this.locationManagers[i].phone = locationManagersVue.locationManagerToBeEdited.phone
								this.locationManagers[i].active = locationManagersVue.locationManagerToBeEdited.active
							}
						}
		...
		validateNewLocationManagerData () {
			...
			return new Promise(function (resolve, reject) {
				...
				} else if (locationManagersVue.newLocationManager.phone.replace(/\D/g, '').length < 10) {
		...
		validateEditedLocationManagerData () {
			...
			return new Promise(function (resolve, reject) {
				if (!locationManagersVue.locationManagerToBeEdited.name.length) {
					reject('Name cannot be blank')
				} else if (locationManagersVue.locationManagerToBeEdited.phone.replace(/\D/g, '').length < 10) {
					reject('Phone number should have at least 10 digits')
				}
				resolve('Hurray')
			})

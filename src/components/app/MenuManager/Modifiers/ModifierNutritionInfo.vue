<template>
	<modal v-bind:show="showNutritionModal" effect="fade" @closeOnEscape="closeModal" ref="modal">
		<div slot="modal-header" class="modal-header">
			<button type="button" class="close" @click="closeModal()">
				<span>&times;</span>
			</button>
			<transition name="fade" mode="out-in">
				<h4 class="modal-title center" v-if="!selectLocationMode" key="mainEditMode">Modifier Item Nutrition Info</h4>
		...
		<div slot="modal-body" class="modal-body"  v-if="!creatingModifierNutritionInfo">
	        <div class="margin-top-20" v-show="errorMessage" ref="errorMessage">
				<div class="alert alert-danger">
	                <span>{{ errorMessage }}</span>
	                <div class="margin-top-15">
	                	<button type="button" class="btn blue" @click="creatingModifierNutritionInfo = true">Add Nutrition Info</button>
	                </div>
	            </div>
	        </div>
    	    <div class="portlet light bg-inverse clear" v-if="!errorMessage.length && !creatingModifierNutritionInfo && !selectLocationMode">
                <div class="portlet-title">
                    <div class="caption">Click the button on the right to edit</div>
                    <div class="actions">
                    	<a class="btn btn-circle btn-icon-only btn-default" @click="editingModifierNutritionInfo = true">
	        	            ...
	        	            <tbody>
	        	                <tr v-for="info in itemNutritionInfo">
	        	                    ...
	        	                    <td v-if="info.key !== 'id' && info.key !== 'modifier_item_id'">
	        	                    	<input type="text" class="form-control input-sm" v-model="info.value" :disabled="!editingModifierNutritionInfo">
	        	                    </td>
	        	                </tr>
	        	            </tbody>
	        	        </table>
		...
		<div slot="modal-body" class="modal-body" v-if="creatingModifierNutritionInfo">
	        <div class="margin-top-20" v-show="createModifierNutritionError" ref="createModifierNutritionError">
				<div class="alert alert-danger">
            		...
            		<div class='table-scrollable table-fixed-height'>
	        	        <table class='table'>
	        	            ...
	        	            <tbody>
	        	                <tr>
	        	                    <td> Calories </td>
	        	                    <td> <input type="text" class="form-control input-sm" v-model="newModifierNutritionInfo.calories"> </td>
	        	                </tr>
	        	                <tr>
	        	                    <td> Minimum Calories </td>
	        	                    <td> <input type="text" class="form-control input-sm" v-model="newModifierNutritionInfo.min_cal"> </td>
	        	                </tr>
	        	                <tr>
	        	                    <td> Total Fat </td>
	        	                    <td> <input type="text" class="form-control input-sm" v-model="newModifierNutritionInfo.total_fat"> </td>
	        	                </tr>
	        	                <tr>
	        	                    <td> Saturated Fat </td>
	        	                    <td> <input type="text" class="form-control input-sm" v-model="newModifierNutritionInfo.saturated_fat"> </td>
	        	                </tr>
	        	                <tr>
	        	                    <td> Trans Fat </td>
	        	                    <td> <input type="text" class="form-control input-sm" v-model="newModifierNutritionInfo.trans_fat"> </td>
	        	                </tr>
	        	                <tr>
	        	                    <td> Cholesterol </td>
	        	                    <td> <input type="text" class="form-control input-sm" v-model="newModifierNutritionInfo.cholesterol"> </td>
	        	                </tr>
	        	                <tr>
	        	                    <td> Sodium </td>
	        	                    <td> <input type="text" class="form-control input-sm" v-model="newModifierNutritionInfo.sodium"> </td>
	        	                </tr>
	        	                <tr>
	        	                    <td> Carbohydrates </td>
	        	                    <td> <input type="text" class="form-control input-sm" v-model="newModifierNutritionInfo.carbs"> </td>
	        	                </tr>
	        	                <tr>
	        	                    <td> Fibre </td>
	        	                    <td> <input type="text" class="form-control input-sm" v-model="newModifierNutritionInfo.fibre"> </td>
	        	                </tr>
	        	                <tr>
	        	                    <td> Sugar </td>
	        	                    <td> <input type="text" class="form-control input-sm" v-model="newModifierNutritionInfo.sugar"> </td>
	        	                </tr>
	        	                <tr>
	        	                    <td> Protein </td>
	        	                    <td> <input type="text" class="form-control input-sm" v-model="newModifierNutritionInfo.protein"> </td>
	        	                </tr>
	        	                <tr>
	        	                    <td> Vitamin A </td>
	        	                    <td> <input type="text" class="form-control input-sm" v-model="newModifierNutritionInfo.vit_a"> </td>
	        	                </tr>
	        	                <tr>
	        	                    <td> Vitamin C </td>
	        	                    <td> <input type="text" class="form-control input-sm" v-model="newModifierNutritionInfo.vit_c"> </td>
	        	                </tr>
	        	                <tr>
	        	                    <td> Calcium </td>
	        	                    <td> <input type="text" class="form-control input-sm" v-model="newModifierNutritionInfo.calcium"> </td>
	        	                </tr>
	        	                <tr>
	        	                    <td> Iron </td>
	        	                    <td> <input type="text" class="form-control input-sm" v-model="newModifierNutritionInfo.iron"> </td>
	        	                </tr>
	        	            </tbody>
	        	        </table>
	        	    </div>
		...
		<div slot="modal-footer" class="modal-footer">
			<button type="button" class="btn btn-primary" v-if="editingModifierNutritionInfo && !creatingModifierNutritionInfo && !selectLocationMode" @click="updateModifierNutritionInfo()">Update</button>
			<button type="button" class="btn btn-primary" v-if="!editingModifierNutritionInfo && creatingModifierNutritionInfo && !selectLocationMode" @click="createModifierNutritionInfo()">Create</button>
			<button type="button" class="btn btn-primary" v-if="!editingModifierNutritionInfo && !creatingModifierNutritionInfo && !selectLocationMode">Save</button>
		</div>
	</modal>
</template>
...
export default {
	...
	methods: {
		...
		getModifierItemNutritionInfo () {
			...
			ModifiersFunctions.getModifierItemNutritionInfo(nutritionInfoVue.modifierItem.id, nutritionInfoVue.$root.appId, nutritionInfoVue.$root.appSecret).then(response => {
		...
		updateModifierNutritionInfo () {
			...
			for (var i = 0; i < nutritionInfoVue.itemNutritionInfo.length; i++) {
				if (nutritionInfoVue.itemNutritionInfo[i].key !== 'modifier_item_id') {
					if (nutritionInfoVue.itemNutritionInfo[i].key === 'id') {
						updatedNutritionInfo.id = nutritionInfoVue.itemNutritionInfo[i].value
					}
			...
			.then(response => {
				...
				ModifiersFunctions.updateModifierNutritionInfo(updatedNutritionInfo, nutritionInfoVue.$root.appId, nutritionInfoVue.$root.appSecret, nutritionInfoVue.$root.userToken).then(response => {
					if (response.code === 200 && response.status === 'ok') {
						this.closeModal()
					}
				}).catch(reason => {
		...
		createModifierNutritionInfo () {
			...
			return nutritionInfoVue.validateNutritionData(nutritionInfoVue.newModifierNutritionInfo)
			.then(response => {
				ModifiersFunctions.createModifierNutritionInfo(nutritionInfoVue.newModifierNutritionInfo, nutritionInfoVue.$root.appId, nutritionInfoVue.$root.appSecret, nutritionInfoVue.$root.userToken).then(response => {
					if (response.code === 200 && response.status === 'ok') {
						this.closeModal()
					}
				}).catch(reason => {

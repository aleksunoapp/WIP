<template>
	<modal :show="showDeleteModifierItemModal" effect="fade" @closeOnEscape="closeModal">
		...
		<div slot="modal-footer" class="modal-footer clear">
			<button type="button" class="btn btn-primary" @click="deleteModifierItem()">Delete</button>
		</div>
	</modal>
</template>
		...
		deleteModifierItem () {
			...
			ModifiersFunctions.deleteModifierItem(deleteModifierItemVue.itemId, deleteModifierItemVue.$root.appId, deleteModifierItemVue.$root.appSecret, deleteModifierItemVue.$root.userToken).then(response => {
				...
				if (reason.responseJSON.code === 401 && reason.responseJSON.status === 'unauthorized') {
					deleteModifierItemVue.$router.push('/login/expired')
					return
				}
				deleteModifierItemVue.errorMessage = reason
				window.scrollTo(0, 0)
			})
		},
		/**

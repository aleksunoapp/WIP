...
<template>
  <div>
    <!-- BEGIN CREATE -->
    <div
      v-if="$root.permissions['stores payment methods create']"
    ...
    >
      ...
      >
        <div class="caption">
          ...
          Create a Non-integrated Payment Method
        </div>
        <div class="tools">
      ...
      >
        ...
        >
          <div class="alert center alert-info">
            ...
            <p>Please select a store from the stores panel on the right to create non-integrated payment methods.</p>
          </div>
        </div>
        <form
          v-show="activeLocationId !== undefined"
    ...
    >
      <div class="portlet-title bg-blue-chambray">
        ...
        <div class="caption">
          ...
          <div class="caption-desc font-grey-cascade">
            Edit or delete non-integrated payment methods.
          </div>
        </div>
      </div>
      <div class="col-md-12">
      ...
      <div class="portlet-body relative-block">
        ...
        <div v-if="activeLocationId === undefined && !loading">
          <div class="alert center alert-info">
            <h4>No Store Selected</h4>
            <p>Please select a store from the stores panel on the right to view non-integrated payment methods for it.</p>
          </div>
        </div>
        <div
            ...
            <ul>
              ...
              >
                ...
                >
                  ...
                  <el-tooltip
                    v-if="$root.permissions['stores payment methods read'] && !$root.permissions['stores payment methods update']"
                    content="View"
                    effect="light"
                    placement="right"
                  >
        ...
        <div class="margin-top-20">
          <no-results
            ...
            :type="'non-integrated payment methods'"
          />
        </div>
      </div>
    </div>
      ...
      >
        <form role="form">
          <div class="row">
            ...
            <div class="col-md-12">
              <fieldset :disabled="!$root.permissions['stores payment methods update']">
                <div class="form-group form-md-line-input form-md-floating-label">
                  <input
...
import PaymentMethodFunctions from '@/controllers/PaymentMethods'
import ajaxErrorHandler from '@/controllers/ErrorController'

export default {
  name: 'NonIntegratedPaymentMethods',
  components: {
    LoadingScreen,
    Modal,
    NoResults
  ...
  methods: {
    ...
    /**
     * To check if the method data is valid before submitting to the backend.
     * @function
     * @returns {object} A promise that will validate the input form
     */
    validateNewMethodData () {
      var _this = this
    ...
    create () {
      ...
      return _this
        ...
        .then(response => {
          ...
          PaymentMethodFunctions.createNonIntegratedPaymentMethod({
            locationId: this.activeLocationId,
            paymentMethod: this.toCreate
          })
            .then(response => {
              if (response.code === 200 && response.status === 'ok') {
                _this.showCreateSuccess(response.payload)
                _this.clearNewMethod()
                _this.getMethods()
              } else {
            ...
            .catch(reason => {
              ajaxErrorHandler({
                reason,
                errorText: 'We could not create the method',
    ...
    showCreateSuccess (payload = {}) {
      ...
      let title = 'Success'
      let text = 'The Method has been created'
      let type = 'success'

      if (payload && payload.pending_approval) {
        title = 'Approval Required'
        text = 'The Method has been sent for approval'
        type = 'info'
      }
    ...
    getMethods () {
      ...
      return PaymentMethodFunctions.listNonIntegratedPaymentMethods({
        locationId: this.activeLocationId
      })
        .then(response => {
        ...
        .catch(reason => {
          ajaxErrorHandler({
            reason,
            errorText: 'We could not fetch the list of non-integrated payment methods',
    ...
    /**
     * To show the modal to edit an method details.
     * @function
     * @param {object} method - The selected method.
     * @returns {undefined}
     */
    edit (method) {
      this.toEdit = { ...method }
    ...
    update () {
      ...
      return _this
        ...
        .then(response => {
          ...
          PaymentMethodFunctions.updateNonIntegratedPaymentMethod({
            locationId: this.activeLocationId,
            paymentMethod: this.toEdit
          })
            .then(response => {
              if (response.code === 200 && response.status === 'ok') {
                _this.getMethods()
            ...
            .catch(reason => {
              ajaxErrorHandler({
                reason,
                errorText: 'We could not update the method',
    ...
    /**
     * To display the modal for deleting an method.
     * @function
     * @param {object} method - The selected method
     * @returns {undefined}
     */
    confirm (method) {
      this.toDelete = { ...method }
    ...
    remove () {
      this.deleting = true
      var _this = this
      return PaymentMethodFunctions.deleteNonIntegratedPaymentMethod({
        locationId: this.activeLocationId,
        paymentMethod: this.toDelete
      })
        .then(response => {
          if (response.code === 200 && response.status === 'ok') {
            _this.getMethods()

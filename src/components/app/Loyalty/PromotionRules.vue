	...
	<div>
		...
		<div class="portlet box blue-hoki margin-top-20">
			...
			<div class="portlet-body" v-show="!createNewCollapse">
				<form role="form" @submit.prevent="createRule()">
					<div class="row">
						<div class="col-md-12">
						...
						<div class="col-md-6">
							...
							<div class="col-xs-12 no-gutter margin-top-15">
								...
								<el-select v-model="newRule.parameter" placeholder="select rule" size="small" class="margin-bottom-15 wide-select">
									...
									<el-option label="purchasing specified items during specified times" value="sku-combination"></el-option>
								</el-select>
							</div>
						</div>
						<div class="col-md-6">
							...
							<div class="col-xs-12 no-gutter">
								<div class="side-by-side">
									...
									<el-select v-model="newRule.bonus_type" placeholder="select type" size="small">
								...
								<div class="form-group form-md-line-input form-md-floating-label side-by-side">
									<input type="text" class="form-control input-sm" v-model="newRule.bonus" :class="{'edited': newRule.bonus.length}">
							...
							<ul>
								...
								>
									...
									<div class="list-item-content height-mod">
										...
										<div class="col-md-4" v-show="rule.parameter === 'sku' || rule.parameter === 'sku-combination'">
											<strong>Applies to: </strong>
											<span v-show="rule.sku === 'all'">all </span><span v-show="rule.sku !== 'all'">{{rule.sku.length}} </span><span>item</span><span v-show="rule.sku.length > 1">s</span>
										</div>
										<div class="col-md-4" v-show="rule.parameter === 'time' || rule.parameter === 'sku-combination'">
	...
	methods: {
		...
		validateNewRule () {
			...
			return new Promise(function (resolve, reject) {
				...
				} else if (new Date(rulesVue.newRule.end_on) < new Date(rulesVue.newRule.start_from)) {
				...
				} else if (rulesVue.newRule.bonus_type === 'multiplier' && !$.isNumeric(rulesVue.newRule.bonus)) {
					reject('Multiplier for bonus points cannot be blank')
				} else if (rulesVue.newRule.bonus_type === 'add-on' && !$.isNumeric(rulesVue.newRule.bonus)) {
					reject('Number of bonus points cannot be blank')
				} else if ((rulesVue.newRule.parameter === 'sku' || rulesVue.newRule.parameter === 'sku') && rulesVue.newRule.parameter.length === 0) {
					reject('Select at least one item')
				} else if ((rulesVue.newRule.bonus_type === 'time' || rulesVue.newRule.parameter === 'sku-combination') && rulesVue.newRule.start.length === 0) {
					reject('Start time cannot be blank')
				} else if ((rulesVue.newRule.parameter === 'time' || rulesVue.newRule.parameter === 'sku-combination') && rulesVue.newRule.end.length === 0) {
		...
		createRule () {
			...
			.then((response) => {
				...
				if (payload.parameter !== 'time' && payload.parameter !== 'sku-combination') {
					payload.start = ''
					payload.end = ''
				}
				rulesVue.clearError('createErrorMessage')

				return LoyaltyFunctions.createPromotionRule(globalFunctions.loyaltyAppId, globalFunctions.loyaltyAppSecret, payload)
				.then(response => {
		...
		getRules () {
			this.loading = true
			const rulesVue = this
			return LoyaltyFunctions.getPromotionRules(globalFunctions.loyaltyAppId, globalFunctions.loyaltyAppSecret)
			.then(response => {
				if (response.code === 200 && response.status === 'ok') {
		...
		validateEditedRule () {
			...
			return new Promise(function (resolve, reject) {
				if (!rulesVue.ruleToEdit.name.length) {
					...
					reject('End date cannot be blank')
				} else if (new Date(rulesVue.ruleToEdit.end_on) < new Date(rulesVue.ruleToEdit.start_from)) {
					reject('End Date cannot be before Start Date')
					...
					reject('Select type of points reward')
				} else if (rulesVue.ruleToEdit.bonus_type === 'multiplier' && !$.isNumeric(rulesVue.ruleToEdit.bonus)) {
					reject('Multiplier for bonus points cannot be blank')
				} else if (rulesVue.ruleToEdit.bonus_type === 'add-on' && !$.isNumeric(rulesVue.ruleToEdit.bonus)) {
					reject('Number of bonus points cannot be blank')
				} else if ((rulesVue.ruleToEdit.parameter === 'sku' || rulesVue.ruleToEdit.parameter === 'sku') && rulesVue.ruleToEdit.parameter.length === 0) {
					reject('Select at least one item')
				} else if ((rulesVue.ruleToEdit.bonus_type === 'time' || rulesVue.ruleToEdit.parameter === 'sku-combination') && rulesVue.ruleToEdit.start.length === 0) {
					reject('Start time cannot be blank')
				} else if ((rulesVue.ruleToEdit.parameter === 'time' || rulesVue.ruleToEdit.parameter === 'sku-combination') && rulesVue.ruleToEdit.end.length === 0) {
					reject('End time cannot be blank')
				}
				resolve('Hurray')
			})
		...
		updateRule () {
			...
			.then((response) => {
				...
				return LoyaltyFunctions.updatePromotionRule(globalFunctions.loyaltyAppId, globalFunctions.loyaltyAppSecret, payload)
				.then(response => {
					if (response.code === 200 && response.status === 'ok') {
						rulesVue.getRules()

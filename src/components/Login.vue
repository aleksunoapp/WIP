...
export default {
	...
	methods: {
		...
		reset (event) {
			...
			.then(response => {
				/* eslint-disable no-undef */
				LoginFunctions.reset(loginVue.token, loginVue.user.email, loginVue.newPassword, loginVue.passwordCheck).then(response => {
		...
		forgot (event) {
			...
			var loginVue = this
			this.clearError()
			return loginVue.validateForgotData()
			.then(response => {
				/* eslint-disable no-undef */
				LoginFunctions.forgot(loginVue.user.email).then(response => {
					// disabledButton.complete()
					this.emailSent = true

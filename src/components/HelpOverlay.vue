...
export default Vue.extend({
  ...
  computed: {
    ...
    tax () {
      return this.formatCurrency(this.$store.state.tax.newlyApproved)
    },
    total () {
      let total = 0
      let formatted = ''
    ...
    setPage (page) {
      ...
      if (page === 2) {
        ...
        if (!input || !label) {
          ...
          if (listScrollContainer) {
            if ((listScrollContainer.scrollHeight - listScrollContainer.clientHeight) > 100) {
              listScrollContainer.scrollTo({
                left: 0,
                top: label.offsetTop + 30,
                behavior: 'smooth'
              })
              this.approve.style.top = 20
              ...
              const offset = this.offset(label)
              this.approve.style.top = offset.top - 70
              this.approve.style.left = offset.left - 10
            }
          }
